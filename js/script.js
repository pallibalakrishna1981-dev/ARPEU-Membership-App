...
function showPage(page) {
  ...
  switch (targetPage) {
   ...
   case "donation":
    case "donations":
      if (rc && rc.getAttribute("data-donation-active") === "true") {
        ...
        if (rc && rc.getAttribute("data-origin-page") === "donations") {
          rc.style.display = "none";
        }
      }
      break;
   ...
   case "stats":
    ...
    case "diary":
    case "diaries":
      if (rc && rc.getAttribute("data-diary-active") === "true") {
        rc.style.display = "block";
        if (diarySec) diarySec.style.display = "none";
      } else {
        if (diarySec) diarySec.style.display = "block";
        if (rc && rc.getAttribute("data-origin-page") === "diary") {
          rc.style.display = "none";
        }
...
if (el) {
  flatpickr(el, { 
    ...
    minDate: id === "#donPayNowDate" ? "today" : undefined,

    // Already Paid: Today and previous dates only
    maxDate: id === "#donPayNowDate"
      ? "today"
      : (id === "#donDate" ? "today" : (id === "#manualDate" ? "today" : undefined)),
...
function toggleDonorTypeFields(selectedType) {
    const memberIdGroup = document.getElementById("donorMemberIdGroup");
    const otherDonorBox = document.getElementById("otherDonorDetailsBox");

    if (!memberIdGroup || !otherDonorBox) return;

    const memberId = document.getElementById("donorMemberId");
    const email = document.getElementById("donorEmail");
    const pan = document.getElementById("donorPan");

    const isMemberEmployee =
        selectedType === "ARPEU Member" ||
        selectedType === "Electricity Employee" ||
        selectedType === "Retired Employee";

    if (isMemberEmployee) {
        memberIdGroup.style.display = "block";
        otherDonorBox.style.display = "none";

        if (email) email.value = "";
        if (pan) pan.value = "";
    } else if (
        selectedType === "Well Wisher / Public" ||
        selectedType === "Shop Owner / Business Establishment" ||
        selectedType === "Business Establishment" ||
        selectedType === "Sponsor / Organization"
    ) {
        memberIdGroup.style.display = "none";
        otherDonorBox.style.display = "block";
...
function handleDonorMobileInput(inputEl) {
  ...
  if (val.length === 10) {
    if (val !== lastDonorSearchedMobile) {
      ...
      searchDonorOrMember(val, document.getElementById("donorType")?.value || "");
    }
  } else {
 ...
 * @param {string} donorType - The type of donor
 */
async function searchDonorOrMember(queryKey, donorType) {
  ...
  try {
    const url = `${targetUrl}?action=searchDonorOrMember&query=${encodeURIComponent(queryKey)}&donorType=${encodeURIComponent(donorType || "")}`;
    const response = await fetch(url);
    const result = await response.json();
...
function applyAutoFillData() {
  ...
  if (document.getElementById("donorType")) toggleDonorTypeFields(document.getElementById("donorType").value);
  ...
  if (d.company && document.getElementById("donorCompany")) document.getElementById("donorCompany").value = d.company;
  if ((d.membershipId || d.memberId) && document.getElementById("donorMemberId")) document.getElementById("donorMemberId").value = d.membershipId || d.memberId;
...
async function submitDonationForm() {
  ...
  const donationPayload = {
    ...
    membershipId:   document.getElementById("donorMemberId") ? document.getElementById("donorMemberId").value.trim() : "",
...
/* ==========================================================
   DONATION RECEIPT GENERATOR (SURGICAL FIX)
   ========================================================== */
function openDonationReceipt(data) {
  ...
  if (tableBody) {
    tableBody.innerHTML = `
      <tr>
        ...
        <td class="amt-col">Rs. <span>${totalAmount}</span></td>
      </tr>
      <tr>
  ...
  if (masterRc) {
    ...
    masterRc.setAttribute("data-donation-active", "true");
    masterRc.setAttribute("data-origin-page", "donations");
    masterRc.removeAttribute("data-membership-active");
  }
...
window.closeReceipt = function () {
  ...
  if (rc) {
    ...
    rc.removeAttribute("data-donation-active");
    rc.removeAttribute("data-diary-active"); // 👉 ఇక్కడ డైరీ యాట్రిబ్యూట్ క్లియర్ అవుతుంది
    rc.removeAttribute("data-receipt-open");
    rc.removeAttribute("data-active");
  }
  ...
  if (originPage === "donations" || originPage === "donation") {
    ...
    if (typeof closeDiaryFlowForm === "function") {
      closeDiaryFlowForm(); // 👉 డైరీ ఫారమ్ ఇన్-పుట్స్ అన్నీ క్లీన్ అయి డ్యాష్‌బోర్డ్ వస్తుంది
    }
    showPage("diary");
  } else {
    if (typeof resetMembershipForm === "function") {
...
const arpeuMasterLeadershipData = {
    ...
    apgenco: {
        ...
        tiers: {
            ...
            nttps: [
                { name: "Sri J. Aswin Kumar", role: "President", company: "CL / CHP / Stage-I", phone: "9959195237", photo: "images/nttps-office-bearers/president-aswin-kumar.jpg" },
                { name: "Sri P. Balakrishna", role: "Secretary", company: "JPA / BM / III / Stage-I", phone: "9642788786", photo: "images/nttps-office-bearers/secretary-balakrishna.jpg" },
                { name: "Sri K. Lakshmi Srinivas", role: "Treasurer", company: "FM Gr.I / Stage-III", phone: "9848182025", photo: "images/nttps-office-bearers/treasurer-lakshmi-srinivas.jpg" },
                { name: "Sri N. Koteswara Rao", role: "Working President", company: "PA / 19ms Shift / Stage-I", phone: "9666987029", photo: "images/nttps-office-bearers/working-president-koteswara-rao.jpg" },
                { name: "Sri K. Laxmi Srinivas", role: "Vice President", company: "FM Gr.I / Stage-III", phone: "9848182025", photo: "images/nttps-office-bearers/vice-president-laxmi-srinivas.jpg" },
                { name: "Sri MD. Imran", role: "Vice President", company: "PA / ESR / BM / Stage-I", phone: "9057986389", photo: "images/nttps-office-bearers/vice-president-imran.jpg" },
                { name: "Smt. K. Suneetha", role: "Vice President", company: "JPA / Colony Maintenance", phone: "8519941632", photo: "images/nttps-office-bearers/vice-president-suneetha.jpg" },
                { name: "Sri P. Rambabu", role: "Vice President", company: "CL / 19ms Shift / Stage-I", phone: "995865814", photo: "images/nttps-office-bearers/vice-president-rambabu.jpg" },
                { name: "Sri V. Suresh", role: "Asst. Secretary", company: "CL / 19ms Shift / Stage-I", phone: "7386680621", photo: "images/nttps-office-bearers/asst-secretary-suresh.jpg" },
                { name: "Sri T. Srinivasa Rao", role: "Asst. Secretary", company: "CL / 19ms Shift / Stage-II", phone: "7596682755", photo: "images/nttps-office-bearers/asst-secretary-srinivasa-rao.jpg" },
                { name: "Sri Ravi Ranastula", role: "Asst. Secretary", company: "CL / SL Gr.-I", phone: "985333734", photo: "images/nttps-office-bearers/asst-secretary-ravi-ranastula.jpg" },
                { name: "Sri K. Sai Kiran", role: "Joint Secretary", company: "CL / 19ms Shift / Stage-I", phone: "6300884476", photo: "images/nttps-office-bearers/joint-secretary-sai-kiran.jpg" },
                { name: "Sri O. Jitendra", role: "Joint Secretary", company: "CL / 19ms Shift / Stage-I", phone: "7996786194", photo: "images/nttps-office-bearers/joint-secretary-jitendra.jpg" },
                { name: "Sri Ch. Hanumaiah", role: "Joint Secretary", company: "Colony Security Guard", phone: "7780549250", photo: "images/nttps-office-bearers/joint-secretary-hanumaiah.jpg" },
                { name: "Sri B.V. Padma Rao", role: "Organising Secretary", company: "CLU / APH / Stage-II", phone: "9909070466", photo: "images/nttps-office-bearers/organising-secretary-padmarao.jpg" }
            ],
            rtpp: [
                { name: "Sri R.V. Ranganath", role: "President", company: "JPA / PHMD / Civil", phone: "8500666571", photo: "images/rtpp-office-bearers/president-ranganath.jpg" },
                { name: "Sri C. Ramagopal Reddy", role: "Secretary", company: "FM Gr.-1 / LOCO CHP", phone: "7989215049", photo: "images/rtpp-office-bearers/secretary-ramagopal-reddy.jpg" },
                { name: "Sri G. Chennakesava Reddy", role: "Treasurer", company: "CLI / ADE / MRT Stg-1", phone: "9908309366", photo: "images/rtpp-office-bearers/treasurer-chennakesava.jpg" },
                { name: "Sri B. Srinivasulu", role: "Working President", company: "CLI / Civil Waterplant", phone: "988595976", photo: "images/rtpp-office-bearers/working-president-srinivasulu.jpg" },
                { name: "Sri R. Venu Gopal", role: "Vice President", company: "JAO / Accounts", phone: "9490136024", photo: "images/rtpp-office-bearers/vice-president-venu-gopal.jpg" },
                { name: "Sri G. Chandramohan Reddy", role: "Vice President", company: "CLI / CHP", phone: "8341178785", photo: "images/rtpp-office-bearers/vice-president-chandramohan.jpg" },
                { name: "Sri R. Rajesh Reddy", role: "Vice President", company: "JPA / CHP / Dozer", phone: "9490979309", photo: "images/rtpp-office-bearers/vice-president-rajesh-reddy.jpg" },
                { name: "Sri L. Jagadish Reddy", role: "Asst. Secretary", company: "MM-1 / CHP", phone: "9014513527", photo: "images/rtpp-office-bearers/asst-secretary-jagadish-reddy.jpg" },
                { name: "Sri U. Vasudevudu", role: "Asst. Secretary", company: "PA CASM SD III / Stage-II", phone: "9494668372", photo: "images/rtpp-office-bearers/asst-secretary-vasudevudu.jpg" },
                { name: "Sri G. C. Ungamaiah", role: "Asst. Secretary", company: "Sr. Asst. / Accounts", phone: "9493353862", photo: "images/rtpp-office-bearers/asst-secretary-ungamaiah.jpg" },
                { name: "Sri Shaik Shabber", role: "Joint Secretary", company: "L/F Cage off coal", phone: "9505870106", photo: "images/rtpp-office-bearers/joint-secretary-shabber.jpg" },
                { name: "Sri P. Subbarayudu", role: "Joint Secretary", company: "CL / COOMN", phone: "9248899470", photo: "images/rtpp-office-bearers/joint-secretary-subbarayudu.jpg" },
                { name: "Sri G. Naresh Kumar Reddy", role: "Joint Secretary", company: "CL / CR / Civil", phone: "9491514292", photo: "images/rtpp-office-bearers/joint-secretary-naresh-kumar.jpg" },
                { name: "Sri M. Harinath", role: "Organising Secretary", company: "Plant / Mechanical", phone: "9966485013", photo: "images/rtpp-office-bearers/organising-secretary-harinath.jpg" },
                { name: "Sri Y. Harichandra Reddy", role: "Office Secretary", company: "CL / CHP / Dozer", phone: "8555877133", photo: "images/rtpp-office-bearers/office-secretary-harichandra.jpg" }
            ],
            sdstps: [
                { name: "Sri Ch. Nagaraju", role: "President", company: "CL / CHP / Stage-I", phone: "9494300764", photo: "images/sdstps-office-bearers/president-nagaraju.jpg" },
                { name: "Sri Ch. Sudhakar", role: "Secretary", company: "CL / CHP / Stage-I", phone: "9704312927", photo: "images/sdstps-office-bearers/secretary-sudhakar.jpg" },
                { name: "Sri Ch. Suresh", role: "Treasurer", company: "CL / EE / Gen. / Stage-I", phone: "8500454563", photo: "images/sdstps-office-bearers/treasurer-suresh.jpg" },
                { name: "Sri K. Sivakumar", role: "Working President", company: "CCR / Shift / Stage -1", phone: "7893211254", photo: "images/sdstps-office-bearers/working-president-sivakumar.jpg" },
                { name: "Sri V. Srinivasulu", role: "Vice President", company: "CL / Store & Fuels Stg - I", phone: "9502377025", photo: "images/sdstps-office-bearers/vice-president-srinivasulu.jpg" },
                { name: "Sri K. Prasad", role: "Vice President", company: "CL / I & C / Stage - 1", phone: "9491277350", photo: "images/sdstps-office-bearers/vice-president-prasad.jpg" },
                { name: "Sri P. Ramesh", role: "Vice President", company: "FM Gr.- IV", phone: "8184974615", photo: "images/sdstps-office-bearers/vice-president-ramesh.jpg" },
                { name: "Sri V. Puttaiah", role: "Asst. Secretary", company: "CL / CHP / Stage-I", phone: "9618624145", photo: "images/sdstps-office-bearers/asst-secretary-puttaiah.jpg" },
                { name: "Sri K. Harikrishna", role: "Asst. Secretary", company: "CIVIL / Stage - I", phone: "7337534260", photo: "images/sdstps-office-bearers/asst-secretary-harikrishna.jpg" },
                { name: "Sri N. Ashok", role: "Asst. Secretary", company: "CL / I & C / Stage - 1", phone: "9701335093", photo: "images/sdstps-office-bearers/asst-secretary-ashok.jpg" },
                { name: "Sri G. Anil", role: "Joint Secretary", company: "CL / Civil / Stg. -I", phone: "8790509445", photo: "images/sdstps-office-bearers/joint-secretary-anil.jpg" },
                { name: "Sri G. Ayyappa", role: "Joint Secretary", company: "CL / CCR / Shift", phone: "9640733211", photo: "images/sdstps-office-bearers/joint-secretary-ayyappa.jpg" },
                { name: "Sri M. Sivasankar", role: "Joint Secretary", company: "CL / WTP / Stage I", phone: "9701694160", photo: "images/sdstps-office-bearers/joint-secretary-sivasankar.jpg" },
                { name: "Sri B. Sudheer", role: "Organising Secretary", company: "CL / CCR SHIFT / Stage -1", phone: "9885425983", photo: "images/sdstps-office-bearers/organising-secretary-sudheer.jpg" },
                { name: "Sri G. Murali", role: "Office Secretary", company: "E&P / Stage -1", phone: "9490078189", photo: "images/sdstps-office-bearers/office-secretary-murali.jpg" }
            ]
        }
    },
    ...
    aptransco: {
        ...
        subTiers: null,
        members: [
            { name: "Sri K. Rajesh", role: "President", company: "DEE / Chittor / Kadapa", phone: "8074602893", photo: "images/transco-office-bearers/president-rajesh.jpg" },
            { name: "Sri N. Dinesh Kumar", role: "Secretary", company: "AEE / Rajampeta", phone: "9490154334", photo: "images/transco-office-bearers/secretary-dinesh-kumar.jpg" },
            { name: "Sri R. Srinivasulu", role: "Treasurer", company: "DEE / Lines / Kadapa", phone: "8074602893", photo: "images/transco-office-bearers/treasurer-srinivasulu.jpg" },
            { name: "Sri K. Narayana", role: "Working President", company: "DEE / Jammalamadugu", phone: "9440817037", photo: "images/transco-office-bearers/working-president-narayana.jpg" },
            { name: "Sri K. Raghava", role: "Vice President", company: "Sub - Engineer / Proj.", phone: "9958541855", photo: "images/transco-office-bearers/vice-president-raghava.jpg" },
            { name: "Sri K. Venkateswarlu", role: "Vice President", company: "Sub - Engineer / Vontimitta", phone: "9958808018", photo: "images/transco-office-bearers/vice-president-venkateswarlu.jpg" },
            { name: "Sri L. Srinivasulu Reddy", role: "Vice President", company: "Sub - Engineer / Rajampeta", phone: "9966781880", photo: "images/transco-office-bearers/vice-president-srinivasulu-reddy.jpg" },
            { name: "Sri G. Vijay Kumar", role: "Assistant Secretary", company: "JLM / Vontimitta", phone: "9966899426", photo: "images/transco-office-bearers/asst-secretary-vijay-kumar.jpg" },
            { name: "Sri P. Narasimha Raju", role: "Assistant Secretary", company: "JLM / Kadapa Telecom", phone: "9631632429", photo: "images/transco-office-bearers/asst-secretary-narasimha-raju.jpg" },
            { name: "Sri V. Sanjeev Kumar", role: "Assistant Secretary", company: "Vontimitta", phone: "9603066659", photo: "images/transco-office-bearers/asst-secretary-sanjeev-kumar.jpg" },
            { name: "Sri S. Thulasi Prasad", role: "Joint Secretary", company: "JLM / Rajampeta", phone: "9441022152", photo: "images/transco-office-bearers/joint-secretary-thulasi-prasad.jpg" },
            { name: "Sri N. Siva Kumar", role: "Joint Secretary", company: "JLM / Vontimitta", phone: "9290611040", photo: "images/transco-office-bearers/joint-secretary-siva-kumar.jpg" },
            { name: "Sri G. Supeera", role: "Joint Secretary", company: "Sub - Engineer / TG Palli SS", phone: "9052248058", photo: "images/transco-office-bearers/joint-secretary-supeera.jpg" },
            { name: "Sri G. Peddaiah", role: "Organising Secretary", company: "Sub-Engineer / Vontimitta", phone: "9925145047", photo: "images/transco-office-bearers/organising-secretary-peddaiah.jpg" },
            { name: "Sri T. Naga Seshaiah", role: "Office Secretary", company: "Watchman / Rajampeta", phone: "9666661964", photo: "images/transco-office-bearers/office-secretary-naga-seshaiah.jpg" }
        ]
    },
    ...
    apspdcl: {
        ...
        tiers: {
            company: [
                { name: "Sri A. Koteswara Rao", role: "President", company: "LM / O / Kavali / Nellore", phone: "9849375974", photo: "images/spdcl-office-bearers/president-koteswara-rao.jpg" },
                { name: "Sri A. Lakshumaiah", role: "Secretary", company: "JAO / Mydikur / Kadapa", phone: "9441684787", photo: "images/spdcl-office-bearers/secretary-lakshmuiah.jpg" },
                { name: "Sri A. Hanok", role: "Treasurer", company: "Jr.Asst. / ERO / Nellore Town", phone: "9394988777", photo: "images/spdcl-office-bearers/treasurer-hanok.jpg" },
                { name: "Sri N. Sivaiah", role: "Working President", company: "LI / O / Atmakur / Nellore", phone: "9652484989", photo: "images/spdcl-office-bearers/working-president-sivaiah.jpg" },
                { name: "Sri S. Devendra Rao", role: "Vice President", company: "LM / West Section / Kadapa", phone: "9666900729", photo: "images/spdcl-office-bearers/vice-president-devendra-rao.jpg" },
                { name: "Smt. C. Karthika", role: "Vice President", company: "Sr. Asst. / SERO / Vontimitta", phone: "9652033371", photo: "images/spdcl-office-bearers/vice-president-karthika.jpg" },
                { name: "Sri B. Mallikarjuna Rao", role: "Vice President", company: "ALM / Jaladanki / Nellore", phone: "996325670", photo: "images/spdcl-office-bearers/vice-president-mallikarjuna-rao.jpg" },
                { name: "Sri P. Suresh", role: "Asst. Secretary", company: "JLM / Kasayanina / Kadapa", phone: "6281605744", photo: "images/spdcl-office-bearers/asst-secretary-suresh.jpg" },
                { name: "Sri D. Charunya", role: "Asst. Secretary", company: "JLM / Chowdavaram / Nellore", phone: "7998973606", photo: "images/spdcl-office-bearers/asst-secretary-charunya.jpg" },
                { name: "Sri D. Ramanaiah", role: "Asst. Secretary", company: "JLM / Mydukur / Kadapa", phone: "9634249093", photo: "images/spdcl-office-bearers/asst-secretary-ramanaiah.jpg" },
                { name: "Sri D. Venkatesh", role: "Joint Secretary", company: "LM / MRT Protection / Annamayya", phone: "9050288800", photo: "images/spdcl-office-bearers/joint-secretary-venkatesh.jpg" },
                { name: "Sri Y. Nagaseshaiah", role: "Joint Secretary", company: "Sr.Asst. / ERO / Rural Kadapa", phone: "8074969670", photo: "images/spdcl-office-bearers/joint-secretary-nagaseshaiah.jpg" },
                { name: "Sri N. Srinivulu Reddy", role: "Joint Secretary", company: "LM / Kondapuram SS / Nellore", phone: "9905407489", photo: "images/spdcl-office-bearers/joint-secretary-srinivulu-reddy.jpg" },
                { name: "Sri S. Masthan Vali", role: "Organising Secretary", company: "Sr.Asst. / ERO / Tadipatri", phone: "9059128456", photo: "images/spdcl-office-bearers/organising-secretary-masthan-vali.jpg" },
                { name: "Sri A. Rajasekhara Rao", role: "Office Secretary", company: "Jr.Asst. / EE / Kavali / Nellore", phone: "9492337692", photo: "images/spdcl-office-bearers/office-secretary-rajasekhara-rao.jpg" }
            ],
            ananthapur: [
                { name: "Sri V. Vidya Sagar", role: "President", company: "Sr. Asst. / ERO / TADIPATRI", phone: "9515126908", photo: "images/ananthapur-office-bearers/president-vidya-sagar.jpg" },
                { name: "Sri J. Pradeep", role: "Working President", company: "JE / Tadipatri", phone: "8317645490", photo: "images/ananthapur-office-bearers/working-president-pradeep.jpg" },
                { name: "Sri S. Masthan Vali", role: "Secretary", company: "Sr. Asst. / Ananthapur / SPDCL", phone: "9059128456", photo: "images/ananthapur-office-bearers/secretary-masthan-vali.jpg" },
                { name: "Sri M. Nagaraju", role: "Organising Secretary", company: "JAO / ERO / KALYANADURGAM", phone: "9441685040", photo: "images/ananthapur-office-bearers/organising-secretary-nagaraju.jpg" },
                { name: "Sri S. MD Shabeer", role: "Treasurer", company: "Jr. Asst. / ERO / TADIPATRI", phone: "9885947549", photo: "images/ananthapur-office-bearers/treasurer-shabeer.jpg" }
            ],
            kadapa: [
                { name: "Sri S. Devendra Rao", role: "President", company: "LM / West Section", phone: "9666990729", photo: "images/kadapa-office-bearers/president-devendra-rao.jpg" },
                { name: "Sri K. Yellam Naidu", role: "Working President", company: "Jr. Asst. / ERO / Kadapa", phone: "6281005452", photo: "images/kadapa-office-bearers/working-president-yellam-naidu.jpg" },
                { name: "Sri K. Gopi Sankar", role: "Secretary", company: "Sr. Asst. / Kadapa", phone: "9866110232", photo: "images/kadapa-office-bearers/secretary-gopi-sankar.jpg" },
                { name: "Sri P. Ayodhya Ramaiah", role: "Organising Secretary", company: "LI / Badvel", phone: "9704928851", photo: "images/kadapa-office-bearers/organising-secretary-ayodhya-ramaiah.jpg" },
                { name: "Sri D. Uttaiah", role: "Treasurer", company: "JLM Gr.-II / VALLUR SECTION", phone: "9502019089", photo: "images/kadapa-office-bearers/treasurer-uttaiah.jpg" }
            ],
            annamayya: [
                { name: "Sri S. Ramachandra", role: "President", company: "ALM / Annamayya", phone: "9154822353", photo: "images/annamayya-office-bearers/president-ramachandra.jpg" },
                { name: "Sri P. Suryanarayana", role: "Working President", company: "JLM / Annamayya", phone: "9849825723", photo: "images/annamayya-office-bearers/working-president-suryanarayana.jpg" },
                { name: "Sri D. Venkatesh", role: "Secretary", company: "LM / MRT Protection", phone: "9059288800", photo: "images/annamayya-office-bearers/secretary-venkatesh.jpg" },
                { name: "Sri R. Sivaprasad", role: "Organising Secretary", company: "CT / METERS-II", phone: "9666626607", photo: "images/annamayya-office-bearers/organising-secretary-sivaprasad.jpg" },
                { name: "Sri B. Sukumar", role: "Treasurer", company: "JLM / Annamayya", phone: "9121654098", photo: "images/annamayya-office-bearers/treasurer-sukumar.jpg" }
            ],
            nellore: [
                { name: "Sri A. Koteswara Rao", role: "President", company: "LM / O / Kavali", phone: "9849375974", photo: "images/nellore-office-bearers/president-koteswara-rao.jpg" },
                { name: "Sri T. Madhava Rao", role: "Working President", company: "LM Gr.-II / MRT", phone: "9441999789", photo: "images/nellore-office-bearers/working-president-madhava-rao.jpg" },
                { name: "Sri A. Hanok", role: "Secretary", company: "Jr.Asst. / ERO / Nellore Town", phone: "9394988777", photo: "images/nellore-office-bearers/secretary-hanok.jpg" },
                { name: "Smt. P. Vijayalakshmi", role: "Organising Secretary", company: "Sr. Asst. / Nellore Town-II", phone: "94914504685", photo: "images/nellore-office-bearers/organising-secretary-vijayalakshmi.jpg" },
                { name: "Sri N. Sivaiah", role: "Treasurer", company: "LI / O / Atmakur Division", phone: "9652048899", photo: "images/nellore-office-bearers/treasurer-sivaiah.jpg" }
            ]
        }
    },
    ...
    apepdcl: {
        ...
        tiers: {
           company: [
                { name: "Sri Antharvedi V. Babu", role: "President", company: "SO / Anakapalli", phone: "9908945833", photo: "images/epdcl-office-bearers/president-antharvedi-babu.jpg" },
                { name: "Sri D. Kiran Kishore", role: "Secretary", company: "JE / Bayyana Gundem / Eluru", phone: "8317645490", photo: "images/epdcl-office-bearers/secretary-kiran-kishore.jpg" },
                { name: "Sri Ch. Naga Raju", role: "Treasurer", company: "AEE / DP 1 / Eluru", phone: "9440816622", photo: "images/epdcl-office-bearers/treasurer-naga-raju.jpg" },
                { name: "Sri G. Gnana Subendra", role: "Working President", company: "JE / O / Tadikalapudi", phone: "9440631328", photo: "images/epdcl-office-bearers/working-president-gnana-subendra.jpg" },
                { name: "Sri T. Nagesh", role: "Vice President", company: "SO / Anakapalli", phone: "7013979699", photo: "images/epdcl-office-bearers/vice-president-nagesh.jpg" },
                { name: "Sri G. Ananta Ramu", role: "Assistant Secretary", company: "SO / Anakapalli", phone: "8501074116", photo: "images/epdcl-office-bearers/asst-secretary-ananta-ramu.jpg" },
                { name: "Sri A. Chandramouli", role: "Joint Secretary", company: "AEDP / Eluru", phone: "6303036662", photo: "images/epdcl-office-bearers/joint-secretary-chandramouli.jpg" },
                { name: "Sri Velisela Ganapathi", role: "Organising Secretary", company: "SO / Anakapalli", phone: "6305882875", photo: "images/epdcl-office-bearers/organising-secretary-ganapathi.jpg" },
                { name: "Sri G. Srikanth", role: "Office Secretary", company: "JE / O / Nidamarru", phone: "8332971279", photo: "images/epdcl-office-bearers/office-secretary-srikanth.jpg" }
            ],
            eluru: [
                { name: "Sri Ch. Nagaraju", role: "President", company: "AE / DPI / Eluru", phone: "9440816622", photo: "images/eluru-office-bearers/president-nagaraju.jpg" },
                { name: "Sri M. Bhanu Prakash", role: "Working President", company: "LM / SS / Bhavanipadu", phone: "8985825986", photo: "images/eluru-office-bearers/working-president-bhanu-prakash.jpg" },
                { name: "Sri K. Gangadhara Rao", role: "Secretary", company: "LM / O / Koppa ku", phone: "8317645490", photo: "images/eluru-office-bearers/secretary-gangadhara-rao.jpg" },
                { name: "Sri G. Ravikumar", role: "Organising Secretary", company: "ALM / O / Bogapuram", phone: "7386244244", photo: "images/eluru-office-bearers/organising-secretary-ravikumar.jpg" },
                { name: "Sri G. Srikanth", role: "Treasurer", company: "JE / VO / Nidadavaru", phone: "8332971279", photo: "images/eluru-office-bearers/treasurer-srikanth.jpg" },
                { name: "Sri P. Devi Pravasudu", role: "Vice President", company: "LM / O / D3 / Eluru", phone: "9141219386", photo: "images/eluru-office-bearers/vice-president-devi-pravasudu.jpg" },
                { name: "Sri A. Chandra Mouli", role: "Asst. Secretary", company: "AE / OP / Eluru", phone: "6303036662", photo: "images/eluru-office-bearers/asst-secretary-chandra-mouli.jpg" },
                { name: "Sri G. Gnana Surendra", role: "Joint Secretary", company: "JE / O / Nidadaipadu", phone: "9440613288", photo: "images/eluru-office-bearers/joint-secretary-gnana-surendra.jpg" },
                { name: "Smt. D. Indra Rani", role: "Office Secretary", company: "JE / O / Tadepalli", phone: "9494285854", photo: "images/eluru-office-bearers/office-secretary-indra-rani.jpg" }
            ],
            anakapalli: [
                { name: "Sri V. Ganapathi", role: "President", company: "SO / Anakapalli", phone: "6305888275", photo: "images/anakapalli-office-bearers/president-ganapathi.jpg" },
                { name: "Sri M. Appa Rao", role: "Working President", company: "SO / Anakapalli", phone: "8106028560", photo: "images/anakapalli-office-bearers/working-president-appa-rao.jpg" },
                { name: "Sri Antharvedi VA Babu", role: "Secretary", company: "SO / Anakapalli", phone: "9908945833", photo: "images/anakapalli-office-bearers/secretary-antharvedi-babu.jpg" },
                { name: "Sri G. Ananta Ramu", role: "Organising Secretary", company: "SO / Anakapalli", phone: "8501074116", photo: "images/anakapalli-office-bearers/organising-secretary-ananta-ramu.jpg" },
                { name: "Sri K. Ramanjaneeyulu", role: "Treasurer", company: "SO / Anakapalli", phone: "8501074116", photo: "images/anakapalli-office-bearers/treasurer-ramanjaneeyulu.jpg" },
                { name: "Sri P. Venkatesh", role: "Vice President", company: "SO / Anakapalli", phone: "6300250335", photo: "images/anakapalli-office-bearers/vice-president-venkatesh.jpg" },
                { name: "Sri DVSR Apalla Naidu", role: "Asst. Secretary", company: "SO / Anakapalli", phone: "9491786889", photo: "images/anakapalli-office-bearers/asst-secretary-apalla-naidu.jpg" },
                { name: "Sri D. Janakiram", role: "Joint Secretary", company: "SO / Anakapalli", phone: "9010446339", photo: "images/anakapalli-office-bearers/joint-secretary-janakiram.jpg" },
                { name: "Sri T. Nagesh", role: "Office Secretary", company: "SO / Anakapalli", phone: "7013979699", photo: "images/anakapalli-office-bearers/office-secretary-nagesh.jpg" }
            ]
        }
    },
    ...
    apcpdcl: {
        ...
        tiers: {
           company: [
                { name: "Sri G. Seetha Ramulu", role: "Hon’ble President", company: "AEP OP / Durgi / Palnadu", phone: "8639022203", photo: "images/cpdcl-office-bearers/hon-president-seetha-ramulu.jpg" },
                { name: "Sri P. Prasad", role: "President", company: "LI / O / Karavadi / Ongole", phone: "9492276307", photo: "images/cpdcl-office-bearers/president-prasad.jpg" },
                { name: "Sri S. Gurubrahmam", role: "Secretary", company: "LI / Tenali Rural / Guntur", phone: "8309195958", photo: "images/cpdcl-office-bearers/secretary-gurubrahmam.jpg" },
                { name: "Sri Ch. Raghu Babu", role: "Treasurer", company: "LM / Ponnuru Rural / Guntur", phone: "9490248069", photo: "images/cpdcl-office-bearers/treasurer-raghu-babu.jpg" },
                { name: "Sri A. Durga Prasad", role: "Working President", company: "Sr. Asst. / Gen / Ongole", phone: "7842190977", photo: "images/cpdcl-office-bearers/working-president-durga-prasad.jpg" },
                { name: "Sri S. Naresh Nanda", role: "Vice President", company: "LM / Guntur", phone: "9299601999", photo: "images/cpdcl-office-bearers/vice-president-naresh-nanda.jpg" },
                { name: "Sri S. Prasanna Kumar", role: "Vice President", company: "SO / Narasaraopet / Palnadu", phone: "9490003898", photo: "images/cpdcl-office-bearers/vice-president-prasanna-kumar.jpg" },
                { name: "Sri M. Venkateswarulu", role: "Vice President", company: "AE / D1 / Operation / Ongole", phone: "7013807999", photo: "images/cpdcl-office-bearers/vice-president-venkateswarulu.jpg" },
                { name: "Sri K. Prathap", role: "Vice President", company: "JLM / MRT / Ongole", phone: "9949612824", photo: "images/cpdcl-office-bearers/vice-president-prathap.jpg" },
                { name: "Sri K. Praveen Kumar", role: "Asst. Secretary", company: "JLM / D3 Tenali / Guntur", phone: "8523853267", photo: "images/cpdcl-office-bearers/asst-secretary-praveen-kumar.jpg" },
                { name: "Sri G. Ramesh Reddy", role: "Asst. Secretary", company: "LI / O / Pallamilli / Ongole", phone: "9948857702", photo: "images/cpdcl-office-bearers/asst-secretary-ramesh-reddy.jpg" },
                { name: "Sri M. Venkata Rao", role: "Asst. Secretary", company: "JLM / Gundayapalem / Ongole", phone: "9949732069", photo: "images/cpdcl-office-bearers/asst-secretary-venkata-rao.jpg" },
                { name: "Sri M. Anil Chowdary", role: "Asst. Secretary", company: "ALM / Pedanandipadu / Ongole", phone: "9949732069", photo: "images/cpdcl-office-bearers/asst-secretary-anil-chowdary.jpg" },
                { name: "Sri P. V. Rayudu", role: "Joint Secretary", company: "ALM / D3 / Narasaraopet", phone: "9989203720", photo: "images/cpdcl-office-bearers/joint-secretary-rayudu.jpg" },
                { name: "Sri S. Venkatesh", role: "Joint Secretary", company: "SO / CRDA", phone: "8885880688", photo: "images/cpdcl-office-bearers/joint-secretary-venkatesh.jpg" },
                { name: "Sri S. Ramesh", role: "Joint Secretary", company: "JLM / Enduru / Ongole", phone: "9133304088", photo: "images/cpdcl-office-bearers/joint-secretary-ramesh.jpg" },
                { name: "Sri S. Giribabu", role: "Joint Secretary", company: "SO / Narasaraopet / Palnadu", phone: "9493911124", photo: "images/cpdcl-office-bearers/joint-secretary-giribabu.jpg" },
                { name: "Sri K. Satayasivaram", role: "Organising Secretary", company: "ALM / Tenali / Guntur", phone: "9395552209", photo: "images/cpdcl-office-bearers/organising-secretary-satayasivaram.jpg" },
                { name: "Sri D. V. Anjaneya Prasad", role: "Office Secretary", company: "JAO / Adm / Ongole", phone: "9848822196", photo: "images/cpdcl-office-bearers/office-secretary-anjaneya-prasad.jpg" }
            
            ],
            crda: [
                { name: "Sri G. Potu Raju", role: "President", company: "ALM / Mangalagiri", phone: "9491664253", photo: "images/crda-office-bearers/president-potu-raju.jpg" },
                { name: "Sri R. Srinivasa Rao", role: "Working President", company: "JLM / Penumuru", phone: "9959089608", photo: "images/crda-office-bearers/working-president-srinivasa-rao.jpg" },
                { name: "Sri V. Gopi Krishna", role: "Secretary", company: "JE / D2 / Mangalagiri", phone: "9701035342", photo: "images/crda-office-bearers/secretary-gopi-krishna.jpg" },
                { name: "Sri K. Narendra Babu", role: "Organising Secretary", company: "JLM / Duagirala", phone: "9160602036", photo: "images/crda-office-bearers/organising-secretary-narendra-babu.jpg" },
                { name: "Sri S. Venkatesh", role: "Treasurer", company: "Shift Operator / Pedapalam", phone: "8885880688", photo: "images/crda-office-bearers/treasurer-venkatesh.jpg" }
            ],
            guntur: [
                { name: "Sri S. Naresh Nanda", role: "President", company: "LM / Guntur", phone: "9299601999", photo: "images/guntur-office-bearers/president-naresh-nanda.jpg" },
                { name: "Sri B. Thandava Krishna", role: "Working President", company: "JLM / Tenali", phone: "9346446999", photo: "images/guntur-office-bearers/working-president-thandava-krishna.jpg" },
                { name: "Sri Ch. Raghu Babu", role: "Secretary", company: "LM / Ponnuru Rural", phone: "9490248069", photo: "images/guntur-office-bearers/secretary-raghu-babu.jpg" },
                { name: "Sri K. Satya Sivaram", role: "Organising Secretary", company: "ALM / Tenali", phone: "9395552209", photo: "images/guntur-office-bearers/organising-secretary-satya-sivaram.jpg" },
                { name: "Sri M. Anil Chowdary", role: "Treasurer", company: "ALM / Pedanandipadu", phone: "9949732069", photo: "images/guntur-office-bearers/treasurer-anil-chowdary.jpg" }
            ],
            palnadu: [
                { name: "Sri S. Prasanna Kumar", role: "President", company: "SO / Narasaraopet", phone: "9490003898", photo: "images/palnadu-office-bearers/president-prasanna-kumar.jpg" },
                { name: "Sri D. Subhani", role: "Working President", company: "ALM / Nakarikallu", phone: "9381160599", photo: "images/palnadu-office-bearers/working-president-subhani.jpg" },
                { name: "Sri P. V. Rayudu", role: "Secretary", company: "ALM / D3 / Narasaraopet", phone: "9989203720", photo: "images/palnadu-office-bearers/secretary-rayudu.jpg" },
                { name: "Sri S. Giribabu", role: "Organising Secretary", company: "SO / Narasaraopet", phone: "9493911124", photo: "images/palnadu-office-bearers/organising-secretary-giribabu.jpg" },
                { name: "Sri M. Babu Naik", role: "Treasurer", company: "ALM / D3 / NRT Rural", phone: "8008231224", photo: "images/palnadu-office-bearers/treasurer-babu-naik.jpg" }
            ],
            prakasham: [
                { name: "Sri K. Prathap", role: "President", company: "JLM / TRE / Ongole", phone: "9949612824", photo: "images/prakasham-office-bearers/president-prathap.jpg" },
                { name: "Sri G. Ramesh", role: "Working President", company: "JLM / Karavadi", phone: "9010471816", photo: "images/prakasham-office-bearers/working-president-ramesh.jpg" },
                { name: "Sri A. Durga Prasad", role: "Secretary", company: "Sr. Asst. / Construction / Ongole", phone: "9059288800", photo: "images/prakasham-office-bearers/secretary-durga-prasad.jpg" },
                { name: "Sri K. Yadukondalu", role: "Organising Secretary", company: "Computer Operator", phone: "6301677240", photo: "images/prakasham-office-bearers/organising-secretary-yadukondalu.jpg" },
                { name: "Sri P. Rangamannar", role: "Treasurer", company: "Jr. Asst. / ERO / Ongole", phone: "9440565280", photo: "images/prakasham-office-bearers/treasurer-rangamannar.jpg" }
            ]
        }
    }
};
...
async function submitDiaryOrderForm() {
    ...
    setTimeout(() => {
        ...
        if (typeof openDonationReceipt === 'function') {
            ...
            // 👉 SURGICAL FIX FOR DIARY ISOLATION
            const diarySec = document.getElementById("diarySection");
            if (diarySec) diarySec.style.display = "none";

            const rc = document.getElementById("receiptContainer");
            if (rc) {
                rc.setAttribute("data-diary-active", "true");
                rc.setAttribute("data-origin-page", "diary");
                rc.removeAttribute("data-donation-active");
                rc.removeAttribute("data-membership-active");
            }
        }
...
function processDiaryOrderPayment(type) {
    ...
    if (typeof openDonationReceipt === 'function') {
        ...
        // 👉 SURGICAL FIX FOR DIARY ADVT ISOLATION
        const diarySec = document.getElementById("diarySection");
        if (diarySec) diarySec.style.display = "none";
